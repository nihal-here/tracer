...
@patch("app.services.investigation_agent.genai.Client")
@patch.dict("os.environ", {"GEMINI_API_KEY": "test-key"})
def test_prompt_contains_evidence_completeness_policy(mock_client_class):
    mock_client = MagicMock()
    mock_client_class.return_value = mock_client
    ...
    choose_next_action("Q?", frozenset(["valid.py"]), [])

    prompt_sent = mock_client.models.generate_content.call_args[1]["contents"]

    # Prove the prompt strictly bounds the FINISH action regarding delegated evidence
    assert "delegates an important requested mechanism to another symbol" in prompt_sent
    assert "follow that dependency" in prompt_sent
    assert "Do not FINISH merely because a plausible answer can be inferred from names" in prompt_sent
    assert "only when the major requested claims are directly evidenced" in prompt_sent
    assert "Avoid exhaustive dependency traversal" in prompt_sent
